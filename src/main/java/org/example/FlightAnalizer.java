...
public class FlightAnalizer {
    public static void main(String[] args) throws Exception {
        if (args.length != 1 || !args[0].endsWith(".json")) {
            System.out.println("Usage: java FlightAnalysisApp path/to/tickets.json");
            return;
        }
        ...
        // List<Ticket> tickets = Arrays.asList(mapper.readValue(jsonFile, Root.class));

        // Фильтрация рейсов Владивостока-Тель-Авива
        ...
        if (!filteredTickets.isEmpty()) {
            ...
            System.out.println("Нет рейсов по маршруту Владивосток-Тель-Авив.");
        }
    }
    ...
    private static Map<String, Integer> calculateMinFlightTimes(List<Ticket> tickets) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd.MM.yy H:mm"); // исправленный формат
        ...
        for (Ticket ticket : tickets) {
            ...
            long flightDurationMinutes = ChronoUnit.MINUTES.between(departure, arrival); // используем правильный класс ChronoUnit
